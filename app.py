# Nama fail: app.py
import streamlit as st
import pandas as pd
from fpdf import FPDF
import datetime
import io  # Diperlukan untuk mengurus fail 'in-memory'
import plotly.express as px # Untuk graf yang lebih baik

# --- FUNGSI-FUNGSI LOGIK (Bahagian "Backend") ---
# --------------------------------------------------

def kira_payroll(senarai_resit):
    """
    Mengambil senarai resit (dari borang) dan mengira gaji.
    """
    KADAR_LORI_PER_KG = 0.07  # RM0.07 sen per kg

    # Ekstrak data dari senarai resit
    jumlah_hasil_jualan = sum(resit['Hasil_RM'] for resit in senarai_resit)
    jumlah_berat_kg = sum(resit['Berat_kg'] for resit in senarai_resit)

    gaji_lori = jumlah_berat_kg * KADAR_LORI_PER_KG
    baki_bersih = jumlah_hasil_jualan - gaji_lori
    gaji_penumbak = baki_bersih / 2
    bahagian_pemilik = baki_bersih / 2
    
    # Kumpul data untuk disimpan ke CSV
    data_kiraan = {
        "jumlah_hasil_jualan": jumlah_hasil_jualan,
        "jumlah_berat_kg": jumlah_berat_kg,
        "jumlah_berat_mt": jumlah_berat_kg / 1000,
        "gaji_lori": gaji_lori,
        "baki_bersih": baki_bersih,
        "gaji_penumbak": gaji_penumbak,
        "bahagian_pemilik": bahagian_pemilik,
        "kadar_lori_per_kg": KADAR_LORI_PER_KG
    }
    return data_kiraan

def jana_pdf_binary(bulan_tahun, senarai_resit, data_kiraan):
    """
    Menjana PDF dan memulangkannya sebagai 'binary data' untuk dimuat turun.
    """
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Helvetica", size=12)

    # --- UBAHSUAI 3: Tajuk PDF Baru ---
    pdf.set_font("Helvetica", 'B', 18) # Saiz lebih besar
    pdf.cell(0, 10, f"LADANG SAWIT SATIN LUNG MANIS BELURAN SABAH", ln=True, align='C')
    # --- Tamat Ubahsuai 3 ---

    pdf.set_font("Helvetica", 'B', 14)
    pdf.cell(0, 10, f"Laporan Kiraan Bayaran - {bulan_tahun}", ln=True, align='C')
    pdf.ln(10) 

    # Bahagian 1: Butiran Jualan
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 10, "Bahagian 1: Butiran Jualan (Resit)", ln=True)
    pdf.set_font("Helvetica", size=11)
    
    for i, resit in enumerate(senarai_resit):
        teks_resit = f"  Resit #{i+1} (Gred {resit['Gred']}): {resit['Berat_kg']:.2f} kg @ RM{resit['Harga_RM_per_MT']:.2f}/MT = RM{resit['Hasil_RM']:.2f}"
        pdf.cell(0, 8, teks_resit, ln=True)
    
    pdf.ln(5)
    pdf.set_font("Helvetica", 'B', 11)
    pdf.cell(0, 8, f"Jumlah Berat Keseluruhan: {data_kiraan['jumlah_berat_kg']:.2f} kg", ln=True)
    pdf.cell(0, 8, f"Jumlah Hasil Jualan Bersih: RM{data_kiraan['jumlah_hasil_jualan']:.2f}", ln=True)
    pdf.ln(10)

    # Bahagian 2: Kiraan Gaji
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 10, "Bahagian 2: Pengiraan Bayaran dan Pembahagian", ln=True)
    
    # Gaji Lori
    pdf.set_font("Helvetica", 'BU', 11)
    pdf.cell(0, 8, "Bayaran Pengangkutan Buah Sawit (Lori):", ln=True)
    pdf.set_font("Helvetica", size=11)
    pdf.cell(0, 8, f"  Kiraan: {data_kiraan['jumlah_berat_kg']:.2f} kg x RM{data_kiraan['kadar_lori_per_kg']:.2f}/kg", ln=True)
    pdf.set_font("Helvetica", 'B', 11)
    pdf.cell(0, 8, f"  Jumlah Gaji Lori = RM{data_kiraan['gaji_lori']:.2f}", ln=True)
    pdf.ln(5)

    # Baki Bersih
    pdf.set_font("Helvetica", 'BU', 11)
    pdf.cell(0, 8, "Hasil Bersih (Selepas Tolak Bayaran Pengangkutan Buah Sawit (Lori)):", ln=True)
    pdf.set_font("Helvetica", size=11)
    pdf.cell(0, 8, f"  Kiraan: RM{data_kiraan['jumlah_hasil_jualan']:.2f} - RM{data_kiraan['gaji_lori']:.2f}", ln=True)
    pdf.set_font("Helvetica", 'B', 11)
    pdf.cell(0, 8, f"  Hasil Bersih = RM{data_kiraan['baki_bersih']:.2f}", ln=True)
    pdf.ln(5)

    # Pembahagian 50/50
    pdf.set_font("Helvetica", 'BU', 11)
    pdf.cell(0, 8, "Pembahagian Hasil Bersih (50/50):", ln=True)
    pdf.set_font("Helvetica", size=11)
    pdf.cell(0, 8, f"  Kiraan: RM{data_kiraan['baki_bersih']:.2f} / 2", ln=True)
    pdf.set_font("Helvetica", 'B', 11)
    pdf.cell(0, 8, f"  Bayaran Pekerja (Penumbak Sawit) = RM{data_kiraan['gaji_penumbak']:.2f}", ln=True)
    pdf.cell(0, 8, f"  Bahagian Pemilik Ladang = RM{data_kiraan['bahagian_pemilik']:.2f}", ln=True)
    pdf.ln(15) # Tambah ruang lebih

    # --- UBAHSUAI 3: Maklumat Penyedia Laporan ---
    pdf.set_font("Helvetica", 'I', 9) # Italic, saiz kecil
    pdf.cell(0, 5, "Laporan ini disediakan oleh:", ln=True, align='L')
    pdf.set_font("Helvetica", 'B', 9) # Bold
    pdf.cell(0, 5, "Mohamad Saifullah Satin", ln=True, align='L')
    pdf.set_font("Helvetica", 'I', 9)
    pdf.cell(0, 5, "Telefon: 019-840 6421", ln=True, align='L')
    pdf.cell(0, 5, "Email: msaifullahsatin@gmail.com", ln=True, align='L')
    
    # Footer tarikh jana
    tarikh_jana = datetime.date.today().strftime("%d-%m-%Y")
    pdf.set_y(-15) # Posisi 1.5 cm dari bawah
    pdf.set_font("Helvetica", 'I', 8)
    pdf.cell(0, 10, f"Laporan dijana secara automatik pada {tarikh_jana}", ln=True, align='C')
    # --- Tamat Ubahsuai 3 ---

    # Output sebagai bytes
    return bytes(pdf.output(dest='S'))

# --- FUNGSI UTAMA APLIKASI WEB (Bahagian "Frontend") ---
# -----------------------------------------------------

st.set_page_config(layout="wide", page_title="Sistem Gaji Sawit")
st.title("Sistem Pengurusan Ladang Sawit 🧑‍🌾")

# --- 1. SIDEBAR: Muat Naik Fail & Navigasi ---
st.sidebar.title("Muat Naik Data")
st.sidebar.info("Sila muat naik fail CSV rekod anda. Jika ini kali pertama, guna fail 'template' kosong.")

uploaded_gaji = st.sidebar.file_uploader("1. Muat Naik `rekod_gaji.csv`", type="csv")
uploaded_jualan = st.sidebar.file_uploader("2. Muat Naik `rekod_jualan.csv`", type="csv")

st.sidebar.title("Navigasi")
page = st.sidebar.radio("Pilih Halaman:", ["📊 Dashboard Statistik", "📝 Kemasukan Data Baru"])

# --- 2. Logik Utama: Memproses Data Yang Dimuat Naik ---
if uploaded_gaji is not None and uploaded_jualan is not None:
    # Simpan data dalam 'session state' supaya tidak hilang bila bertukar halaman
    try:
        # Guna 'BytesIO' untuk membaca fail yang dimuatnaik
        st.session_state.df_gaji = pd.read_csv(io.BytesIO(uploaded_gaji.getvalue()))
        st.session_state.df_jualan = pd.read_csv(io.BytesIO(uploaded_jualan.getvalue()))
    except pd.errors.EmptyDataError:
        # Jika fail kosong (pertama kali), cipta DataFrame kosong dengan kolum yang betul
        st.session_state.df_gaji = pd.DataFrame(columns=['BulanTahun', 'JumlahJualan_RM', 'JumlahBerat_kg', 'GajiLori_RM', 'GajiPenumbak_RM', 'BahagianPemilik_RM'])
        st.session_state.df_jualan = pd.DataFrame(columns=['BulanTahun', 'IDResit', 'Gred', 'Berat_kg', 'Harga_RM_per_MT', 'Hasil_RM'])
    except Exception as e:
        st.error(f"Ralat semasa membaca fail CSV: {e}")
        st.stop() # Hentikan aplikasi jika fail rosak

    # --- Halaman 1: Dashboard Statistik ---
    if page == "📊 Dashboard Statistik":
        st.header("📊 Dashboard Statistik")
        
        if st.session_state.df_gaji.empty:
            st.warning("Tiada data untuk dipaparkan. Sila ke halaman 'Kemasukan Data Baru' untuk menambah data.")
        else:
            df_gaji = st.session_state.df_gaji
            df_jualan = st.session_state.df_jualan
            
            # KPI Utama
            total_sales = df_gaji['JumlahJualan_RM'].sum()
            total_weight_kg = df_gaji['JumlahBerat_kg'].sum()
            avg_monthly_owner = df_gaji['BahagianPemilik_RM'].mean()

            col1, col2, col3 = st.columns(3)
            col1.metric("Jumlah Jualan Keseluruhan", f"RM{total_sales:,.2f}")
            col2.metric("Jumlah Berat Keseluruhan", f"{total_weight_kg:,.0f} kg")
            col3.metric("Purata Pendapatan Bulanan (Pemilik)", f"RM{avg_monthly_owner:,.2f}")
            
            st.markdown("---")
            
            # Graf Tren
            st.subheader("Tren Jualan dan Pembahagian Gaji")
            df_gaji_sorted = df_gaji.copy()
            try:
                df_gaji_sorted['TarikhSort'] = pd.to_datetime(df_gaji_sorted['BulanTanun'], format='%B %Y', errors='coerce')
                if df_gaji_sorted['TarikhSort'].isnull().all(): 
                    df_gaji_sorted['TarikhSort'] = pd.to_datetime(df_gaji_sorted['BulanTahun'], errors='coerce')
                df_gaji_sorted = df_gaji_sorted.sort_values('TarikhSort')
            except Exception:
                pass 
                
            fig_tren_gaji = px.line(
                df_gaji_sorted, 
                x='BulanTahun', 
                y=['JumlahJualan_RM', 'GajiLori_RM', 'GajiPenumbak_RM', 'BahagianPemilik_RM'],
                title="Perbandingan Jualan Kasar vs Pembahagian Gaji",
                labels={'value': 'Jumlah (RM)', 'BulanTahun': 'Bulan'},
                markers=True
            )
            st.plotly_chart(fig_tren_gaji, use_container_width=True)
            
            # Analisis Gred
            st.subheader("Analisis Mengikut Gred")
            col_gred1, col_gred2 = st.columns(2)
            fig_pie_hasil = px.pie(
                df_jualan, names='Gred', values='Hasil_RM', 
                title="Pecahan Hasil Jualan (RM) mengikut Gred"
            )
            col_gred1.plotly_chart(fig_pie_hasil, use_container_width=True)
            
            fig_pie_berat = px.pie(
                df_jualan, names='Gred', values='Berat_kg', 
                title="Pecahan Berat Jualan (kg) mengikut Gred"
            )
            col_gred2.plotly_chart(fig_pie_berat, use_container_width=True)
            
            st.markdown("---")
            st.subheader("Data Mentah")
            st.dataframe(df_gaji)
            st.dataframe(df_jualan)

    # --- Halaman 2: Kemasukan Data Baru ---
    elif page == "📝 Kemasukan Data Baru":
        st.header("📝 Kemasukan Data Jualan Bulanan Baru")
        
        with st.form("borang_data_baru"):
            
            st.subheader("1. Maklumat Asas")
            
            # --- UBAHSUAI 4: Dropdown untuk Bulan dan Tahun ---
            col1, col2 = st.columns(2)
            
            senarai_bulan = ["Januari", "Februari", "Mac", "April", "Mei", "Jun", 
                            "Julai", "Ogos", "September", "Oktober", "November", "Disember"]
            
            # Dapatkan tahun semasa untuk senarai pilihan
            tahun_semasa = datetime.date.today().year
            # Buat senarai dari 5 tahun lepas hingga 1 tahun akan datang
            senarai_tahun = list(range(tahun_semasa - 5, tahun_semasa + 2)) 
            senarai_tahun.reverse() # Letak tahun terbaru di atas

            with col1:
                # Default kepada bulan semasa
                bulan = st.selectbox("Pilih Bulan:", senarai_bulan, index=datetime.date.today().month - 1) 
            
            with col2:
                # Default kepada tahun semasa
                tahun = st.selectbox("Pilih Tahun:", senarai_tahun)

            # Gabungkan jadi satu string
            bulan_tahun = f"{bulan} {tahun}"
            
            # Papar untuk pengesahan
            st.info(f"Anda sedang memasukkan data untuk: **{bulan_tahun}**")
            # --- Tamat Ubahsuai 4 ---
            
            st.subheader("2. Butiran Resit Jualan")
            st.info("Gunakan editor di bawah untuk memasukkan semua resit anda. Klik baris akhir dan tekan 'Enter' atau klik ikon '+' untuk menambah baris (resit) baru.")
            
            # --- UBAHSUAI 1: Tambah Gred C dalam data contoh ---
            df_resit_input = pd.DataFrame(
                [
                    {"Gred": "A", "Berat_kg": 0.0, "Harga_RM_per_MT": 0.0},
                    {"Gred": "B", "Berat_kg": 0.0, "Harga_RM_per_MT": 0.0},
                    {"Gred": "C", "Berat_kg": 0.0, "Harga_RM_per_MT": 0.0}, # Tambah ini
                ]
            )
            # --- Tamat Ubahsuai 1 ---
            
            edited_df = st.data_editor(
                df_resit_input,
                num_rows="dynamic", # Benarkan pengguna menambah/memadam baris
                column_config={
                    "Gred": st.column_config.SelectboxColumn(
                        "Gred",
                        # Pilihan "C" sememangnya sudah ada di sini
                        options=["A", "B", "C"],
                        required=True,
                    ),
                    "Berat_kg": st.column_config.NumberColumn(
                        "Berat (kg)",
                        min_value=0.0,
                        format="%.2f",
                        required=True,
                    ),
                    "Harga_RM_per_MT": st.column_config.NumberColumn(
                        "Harga Jualan (RM/MT)",
                        min_value=0.0,
                        format="%.2f",
                        required=True,
                    )
                },
                key="data_editor"
            )
            
            submit_button = st.form_submit_button(label="Kira, Jana PDF & Simpan Data")

        # --- Logik Selepas Borang Dihantar ---
        if submit_button:
            # Semakan keselamatan
            if not bulan_tahun:
                st.error("Ralat: Sila pilih Bulan dan Tahun.")
            elif edited_df['Berat_kg'].sum() == 0:
                st.error("Ralat: Sila masukkan sekurang-kurangnya satu resit jualan dengan berat lebih dari 0.")
            # Semak jika data bulan itu sudah wujud
            elif bulan_tahun in st.session_state.df_gaji['BulanTahun'].values:
                st.error(f"Ralat: Data untuk {bulan_tahun} sudah wujud dalam rekod anda. Sila muat naik fail CSV yang betul atau pilih bulan lain.")
            else:
                with st.spinner("Sedang mengira..."):
                    
                    # 1. Bersihkan data resit (ambil hanya baris yang ada data)
                    senarai_resit = edited_df[edited_df['Berat_kg'] > 0].to_dict('records')
                    
                    # 2. Kira hasil untuk setiap resit
                    for i, resit in enumerate(senarai_resit):
                        resit['Hasil_RM'] = (resit['Berat_kg'] / 1000) * resit['Harga_RM_per_MT']
                        resit['BulanTahun'] = bulan_tahun
                        resit['IDResit'] = i + 1

                    # 3. Lakukan kiraan gaji
                    data_kiraan = kira_payroll(senarai_resit)
                    
                    # 4. Jana PDF (dalam ingatan)
                    pdf_binary = jana_pdf_binary(bulan_tahun, senarai_resit, data_kiraan)
                    
                    # 5. Sediakan data baru untuk ditambah ke CSV
                    df_gaji_baru = pd.DataFrame([{
                        'BulanTahun': bulan_tahun,
                        'JumlahJualan_RM': data_kiraan['jumlah_hasil_jualan'],
                        'JumlahBerat_kg': data_kiraan['jumlah_berat_kg'],
                        'GajiLori_RM': data_kiraan['gaji_lori'],
                        'GajiPenumbak_RM': data_kiraan['gaji_penumbak'],
                        'BahagianPemilik_RM': data_kiraan['bahagian_pemilik']
                    }])
                    
                    df_jualan_baru = pd.DataFrame(senarai_resit)[
                        ['BulanTahun', 'IDResit', 'Gred', 'Berat_kg', 'Harga_RM_per_MT', 'Hasil_RM']
                    ]
                    
                    # 6. Gabungkan data lama dan baru
                    df_gaji_updated = pd.concat([st.session_state.df_gaji, df_gaji_baru], ignore_index=True)
                    df_jualan_updated = pd.concat([st.session_state.df_jualan, df_jualan_baru], ignore_index=True)
                    
                    # 7. Simpan semula ke session_state (supaya dashboard akan update)
                    st.session_state.df_gaji = df_gaji_updated
                    st.session_state.df_jualan = df_jualan_updated

                    # 8. Papar rumusan & butang muat turun
                    st.success(f"Data untuk {bulan_tahun} telah berjaya diproses!")
                    
                    st.subheader("Rumusan Kiraan")
                    st.metric("Jumlah Jualan Kasar", f"RM{data_kiraan['jumlah_hasil_jualan']:.2f}")
                    st.metric("Gaji Pekerja 1 (Lori)", f"RM{data_kiraan['gaji_lori']:.2f}")
                    st.metric("Gaji Pekerja 2 (Penumbak)", f"RM{data_kiraan['gaji_penumbak']:.2f}")
                    st.metric("Bahagian Pemilik", f"RM{data_kiraan['bahagian_pemilik']:.2f}")
                    
                    st.subheader("Muat Turun")
                    nama_fail_pdf = f"Laporan_Gaji_{bulan_tahun.replace(' ', '_')}.pdf"
                    st.download_button(
                        label="1. Muat Turun Laporan PDF",
                        data=pdf_binary,
                        file_name=nama_fail_pdf,
                        mime="application/pdf"
                    )
                    
                    st.download_button(
                        label="2. Muat Turun `rekod_gaji.csv` (Terkini)",
                        data=df_gaji_updated.to_csv(index=False).encode('utf-8'),
                        file_name="rekod_gaji.csv",
                        mime="text/csv",
                        key="download_gaji"
                    )
                    
                    st.download_button(
                        label="3. Muat Turun `rekod_jualan.csv` (Terkini)",
                        data=df_jualan_updated.to_csv(index=False).encode('utf-8'),
                        file_name="rekod_jualan.csv",
                        mime="text/csv",
                        key="download_jualan"
                    )
                    
                    st.warning("PENTING: Sila muat turun dan simpan kedua-dua fail CSV (Terkini) untuk kegunaan bulan hadapan.")

else:
    # Paparan jika fail belum dimuat naik
    st.info("Sila mulakan dengan memuat naik fail `rekod_gaji.csv` dan `rekod_jualan.csv` anda di sidebar kiri.")